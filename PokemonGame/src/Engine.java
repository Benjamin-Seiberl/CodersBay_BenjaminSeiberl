...
public class Engine {
    public static void main(String[] args) {
       ...
       Pokedex pokedex = database.fillPokedex();

       //lookupGameState
        ArrayList<Integer> enemies = csv.lookUpGameState();
        if(enemies.size() == 4){
            printIntro();
        }
        Trainer player = database.getTrainer(1);
        printSaveData(enemies, player);

        //main game loop
        for (int i = 0; i < enemies.size(); i++) {
            Trainer enemy = database.getTrainer(enemies.get(i));
            ...
            if(won){
                if(i == enemies.size()-1){
                    ...
                    if (!continueB) {
                        database.saveData(enemies.get(i), enemies);
                        database.updateTrainer(player);
                        break;
                    }
                }
            }else{
